...
async function launchMeetingProcessorAgent() {
  try {
    const { activationMessage } = launchAgentWithPrompt('meeting-processor-agent');

    // Launch Claude Code with the meeting processor agent message and prompt
    ...
    await new Promise((resolve, reject) => {
      ...
      process.on('SIGINT', () => {
        console.log('\n🛑 Stopping Meeting Processor Agent...');
    ...
    console.error('❌ Error launching Meeting Processor Agent:', error.message);
    process.exit(1);
  }
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  launchMeetingProcessorAgent();
}

export { launchMeetingProcessorAgent };
