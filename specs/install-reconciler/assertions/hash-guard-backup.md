...
- `backupFile` never fails the install over the backup name. A candidate name that holds a file it cannot compare, a directory, or a symlink is simply taken, and the next name is used. A backup is never written through a symlink another tool made.
