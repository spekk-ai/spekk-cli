...
- If no skill is provided or the first arg starts with `-` (flag) or doesn't match any skill, observer falls back to its existing flag-parsing behavior (`--quiet`, `--headless`, `--claude-path`)
- Skill invocation does not break existing observer flag behavior — `spekk observer --quiet` still works
