...
**Self-documenting header (required):**

Every spec `.md` and every assertion `.md` you write begins with an
HTML-comment header, placed immediately after the closing `---` of the
frontmatter and before the `# ` title. It renders invisibly on GitHub but
stays visible in raw view and editors, so a cold reader opening a single
file understands the frontmatter fields without leaving the file. Keep it
terse — one phrase per field, no elaboration — since `specs/README.md` is
the fuller reference. Copy the literal block below (do not paraphrase it).
...
<!--
  ...
  branch   — git branch this spec's work lives on (optional; omit for main)
  status   — not stored here; computed from child assertions, see below
  See specs/README.md for the full concept + frontmatter reference.
-->
```

Assertion file header:
```
<!--
