...
- The documentation warns against scraping the token out of the terminal at all: the command prints it through a redrawing interface, so the byte stream contains characters that were later overwritten, and no escape-stripping pattern reconstructs the screen. It states that the resulting token is the right length and shape and fails only at first use with a 401, and that the reliable check is to authenticate with the captured value locally before deploying it.
