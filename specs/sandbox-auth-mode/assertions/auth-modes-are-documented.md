...
- The documentation warns that a token captured from a terminal can be truncated by line wrapping, that the result is well-formed and fails only at first use with a 401, and that the defenses are a wide terminal and a length check rather than a prefix match.
