...
**Tests:** `internal/observation/` — a repository with one observation resolved on main and one unrelated open observer branch cut from that main: the inherited copy does not cover, and the open finding on its own branch still does; a branch that carries an observation named after a different branch does not cover; a branch merged to main but not deleted does not cover. `internal/observation/` — the digest semantics are pinned separately over a synthetic union.
