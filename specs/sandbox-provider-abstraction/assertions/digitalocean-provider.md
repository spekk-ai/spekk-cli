...
- `DOProvider.Create` records what it resolved, not what the flags said: omitting `--region` and `--size` still leaves `nyc1` and `s-2vcpu-4gb` in metadata, and a project named by UUID is recorded by name.
