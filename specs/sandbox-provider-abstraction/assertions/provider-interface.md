...
- `Destroy` always calls the provider, and returns its error. It never removes local metadata after a failed teardown, because that metadata is what makes an orphaned machine findable. The one exception is a forced destroy of a record that names no machine, which `legacy-metadata-stays-destroyable` defines.
