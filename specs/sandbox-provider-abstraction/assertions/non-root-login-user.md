...
- The privileged steps - credential injection, git credentials, agent deploy, teardown - use one helper to run their script with the required privileges. Deployment prepares and replaces the staged binary inside that script. A root login runs the script directly.
