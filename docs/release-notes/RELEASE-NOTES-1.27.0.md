...
The steps that need root are the same three as before - inject credentials, deploy the agent, tear it down - and each escalates with `sudo`, so the login user needs passwordless sudo. Everything else runs unprivileged. A sandbox recorded before this release has no login user, which reads as root, so nothing changes for one that already exists.
