...
`)
	...
	case "status":
		...
		meta, err := sandbox.GetSandbox(subArgs[0])
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: %s\n", err)
			os.Exit(1)
		}
		if meta == nil {
			fmt.Fprintf(os.Stderr, "Error: sandbox %q not found\n", subArgs[0])
			os.Exit(1)
		}
		p, err := sandbox.ProviderFromMeta(meta)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: %s\n", err)
			os.Exit(1)
		}
	...
	case "destroy":
		...
		meta, err := sandbox.GetSandbox(name)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error: %s\n", err)
			os.Exit(1)
		}
